...
`;function it(){if(document.getElementById("cv-transitions"))return;const e=document.createElement("style");e.id="cv-transitions",e.textContent=ot,document.head.appendChild(e)}async function re(e,t,r){e.classList.add(`${t}-${r}`);const v=getComputedStyle(e),u=Math.max(parseFloat(v.animationDuration)||0,parseFloat(v.transitionDuration)||0)*1e3;u>0&&await new Promise(y=>{const n=()=>y();e.addEventListener("animationend",n,{once:!0}),e.addEventListener("transitionend",n,{once:!0}),setTimeout(n,u+50)}),e.classList.remove(`${t}-${r}`)}var ce=new Map;async function ct(e){if(typeof e!="function")return e;if(ce.has(e))return ce.get(e);const t=await e();return ce.set(e,t.default),t.default}function De(e,t){if(e.components)return e.components[t];if(t==="default")return e.component}function Pe(e,t){if(e==="*")return{};const r=[],v=e.replace(/:(\w+)/g,(y,n)=>(r.push(n),"([^/]+)")),u=t.match(new RegExp(`^${v}$`));return u?Object.fromEntries(r.map((y,n)=>[y,u[n+1]])):null}function lt(e,t){if(e==="/")return{params:{},remaining:t};const r=[],v=e.replace(/:(\w+)/g,(y,n)=>(r.push(n),"([^/]+)")),u=t.match(new RegExp(`^${v}(/.+)?$`));return u?{params:Object.fromEntries(r.map((y,n)=>[y,u[n+1]])),remaining:u[r.length+1]||"/"}:null}var Q=(e,t)=>new Promise(r=>e(t,r)),He=(e,t)=>e?.beforeLeave?new Promise(r=>e.beforeLeave(t,r)):Promise.resolve(void 0);function dt(e,t){return t?e===t?"/":e.startsWith(t+"/")?e.slice(t.length)||"/":e||"/":e||"/"}function Re(e){if(!e)return{};const t=new URLSearchParams(e.startsWith("?")?e.slice(1):e),r={};return t.forEach((v,u)=>{r[u]=v}),r}function Fe(e,t,r,v="default",u){const y=t.base??"",n=g=>g.length>1&&g.endsWith("/")?g.slice(0,-1):g,w=()=>t.mode==="history"?n(dt(window.location.pathname,y)):n((window.location.hash.slice(1)||"/").split("?")[0]||"/"),O=()=>{if(t.mode==="history")return Re(window.location.search);const g=window.location.hash.slice(1)||"/",l=g.indexOf("?");return l>=0?Re(g.slice(l+1)):{}};t.transition&&it();let M=null,L=null,i=null,s=null,a=!1;const o=()=>{a||(a=!0,u?.())},c=new Map,m=g=>{if(g?.keepAlive&&i){i.deactivate?.();const l=document.createDocumentFragment();for(;e.firstChild;)l.appendChild(e.firstChild);c.set(M.path,{fragment:l,activation:i}),i=null}else i?.destroy(),i=null,e.innerHTML=""},d=async(g,l,f,$,h)=>{const E=typeof l=="function"&&!ce.has(l),k=E?l.__asyncOptions:void 0,S=g.loadingTemplate??k?.loadingTemplate;E&&S&&(e.innerHTML=S);let z;try{z=await ct(l)}catch(A){const x=k?.errorTemplate;if(x)return e.innerHTML=x,{destroy:()=>{e.innerHTML=""}};throw A}return E&&S&&(e.innerHTML=""),r(e,z,f,$,h)},p=async()=>{const g=w(),l=O();for(const f of t.routes){if(f.children?.length){const h=lt(f.path,g);if(h!==null)for(const E of f.children){const k=Pe(E.path,g);if(k!==null){const S={params:h.params,query:l,path:g,meta:f.meta};if(E.redirect){const C={params:k,query:l,path:g,meta:E.meta},_=typeof E.redirect=="function"?E.redirect(C):E.redirect;t.navigate(_);return}if(t.beforeEach){const C=await Q(t.beforeEach,S);if(C){t.navigate(C);return}}if(f.beforeEnter){const C=await Q(f.beforeEnter,S);if(C){t.navigate(C);return}}if(E.beforeEnter){const C={params:k,query:l,path:g,meta:E.meta},_=await Q(E.beforeEnter,C);if(_){t.navigate(_);return}}const z=`${f.path}::${JSON.stringify(h.params)}`;if(s!==z){const C=await He(i,S);if(C){t.navigate(C);return}const _=f.transition??t.transition;_&&e.hasChildNodes()&&await re(e,_,"leave"),m(L);const H=De(f,v);if(H){const D={routes:f.children,mode:t.mode,base:t.base,transition:f.transition??t.transition,beforeEach:t.beforeEach,afterEach:t.afterEach,scrollBehavior:t.scrollBehavior,navigate:(R,N)=>t.navigate(R,N),replace:(R,N)=>t.replace(R,N),back:()=>t.back(),forward:()=>t.forward()};i=await d(f,H,S,v==="default"?f.layout:void 0,v==="default"?D:void 0),i.enter?.(M)}else e.innerHTML="";s=z,_&&await re(e,_,"enter")}const A={params:{...h.params,...k},query:l,path:g,meta:E.meta??f.meta};t.afterEach?.(A,M);const x=t.scrollBehavior?.(A,M);x&&window.scrollTo(x.x??0,x.y??0),M=A,L=f,o();return}}}const $=Pe(f.path,g);if($!==null){s=null;const h={params:$,query:l,path:g,meta:f.meta};if(f.redirect){const A=typeof f.redirect=="function"?f.redirect(h):f.redirect;t.navigate(A);return}if(t.beforeEach){const A=await Q(t.beforeEach,h);if(A){t.navigate(A);return}}if(f.beforeEnter){const A=await Q(f.beforeEnter,h);if(A){t.navigate(A);return}}const E=await He(i,h);if(E){t.navigate(E);return}const k=f.transition??t.transition;k&&e.hasChildNodes()&&await re(e,k,"leave"),m(L);const S=De(f,v);if(S){const A=h.path;if(f.keepAlive&&c.has(A)){const x=c.get(A);e.appendChild(x.fragment),i=x.activation,i.activate?.(),c.delete(A)}else{const x=M;i=await d(f,S,h,v==="default"?f.layout:void 0),i.enter?.(x)}}else e.innerHTML="",i=null;k&&await re(e,k,"enter"),t.afterEach?.(h,M);const z=t.scrollBehavior?.(h,M);z&&window.scrollTo(z.x??0,z.y??0),M=h,L=f,o();return}}s=null,m(L),L=null,o()},b=t.mode==="history"?"popstate":"hashchange";return window.addEventListener(b,p),p(),()=>{window.removeEventListener(b,p),i?.destroy(),i=null,c.forEach(({activation:g})=>g.destroy()),c.clear()}}function ut(){if(typeof window>"u")return null;if(window.__COURVUX_DEVTOOLS__)return window.__COURVUX_DEVTOOLS__;const e=new Map,t={instances:[],stores:[],on(r,v){return e.has(r)||e.set(r,new Set),e.get(r).add(v),()=>e.get(r)?.delete(v)},_emit(r,v){e.get(r)?.forEach(u=>{try{u(v)}catch{}})},_registerInstance(r){this.instances.push(r),this._emit("mount",r)},_unregisterInstance(r){const v=this.instances.findIndex(u=>u.id===r);if(v!==-1){const u=this.instances[v];this.instances.splice(v,1),this._emit("destroy",u)}},_registerStore(r){this.stores.push(r),r.subscribe(()=>this._emit("store-update",r))}};return window.__COURVUX_DEVTOOLS__=t,t}var pt=0;function ft(){return++pt}var mt=`
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const zt=[["rect",{width:"18",height:"11",x:"3",y:"11",rx:"2",ry:"2"}],["path",{d:"M7 11V7a5 5 0 0 1 10 0v4"}]];/**
 * @license lucide v1.14.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ct=[["path",{d:"M4 5h16"}],["path",{d:"M4 12h16"}],["path",{d:"M4 19h16"}]];/**
 * @license lucide v1.14.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
